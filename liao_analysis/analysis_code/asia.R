...
## 免簽
tour_visa_country <- tour %>%
    filter(grepl("日本|韓國|歐洲地區|美國",Location)) %>%
    mutate(Year=Year+1911) %>%
    filter(Year>=2000)
# write_csv(tour_visa_country, "C:/Users/user/rlads_communicate/communicate.web.source/currency/visa_country_tour.csv")
