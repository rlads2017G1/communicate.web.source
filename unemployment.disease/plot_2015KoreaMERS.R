...
tour_korea_2015 <- left_join(tour_japan,tour_korea,by="YMD") %>% mutate(Japan=Japan/10000, Korea=Korea/10000)
...
pl_MERS_tour <- dygraph(tour_korea_2015, main = "台灣前往日/韓之旅遊人數", group = "MERS") %>%
...
pl_MERS_case <- dygraph(mers_month, main = "MERS in Korea", group = "MERS") %>%
    dyOptions(axisLabelFontSize = 12, axisLineWidth = 0.8, drawGrid=F) %>%
    dySeries("month_death", axis = 'y', label = "死亡人數",strokeWidth = 1.5,color=hue_pal()(4)[4]) %>%
    dySeries("month_case", axis = 'y', label = "病例數",strokeWidth = 1.5,color = hue_pal()(4)[3]) %>%
    dyAxis("y", label = "人數") %>%
